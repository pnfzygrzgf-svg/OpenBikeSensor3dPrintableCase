...
// Oder relativ zu deinem Projektordner, z.B.:
top_case_path = "top_case.stl";
...
// Importiertes Top Case
module TopCase() {
  import(top_case_path);
}

// Top Case positioniert auf der MountRail
module PositionedTopCase() {
  translate([0, SeatPostMount_stop_plate_thickness + MountRail_width/2, MountRail_total_height])
  rotate([90, 0, 0])
  translate([0, 0, 23.6])  // Z-Offset korrigieren
  TopCase();
}
...
if (orient_for_printing) {
  ...
  rotate([0, -90, -180]) {
    SeatPostMount();
    color("lightblue", 0.8) PositionedTopCase();
  }
}